...
spl_autoload_register( function( $class ) {
		...
		if ( defined('LIBS_DIR') && file_exists( LIBS_DIR.$class.'.php' ) ) {
...
function find_custom_recursive(&$excludes, $array = array(), $searched_key = '', $returned_key = 0) {
	$tmp_array = array();
	if (is_array($array) && !empty($array)) {
		foreach($array as $key=>$value) {
			if(in_array($key, $excludes))
				continue;
			if(is_array($value)) {
				$returned = find_custom_recursive($excludes, $value, $searched_key, $returned_key);
...
function create_translate_files($translation_dir, $json_dir, $option_prefix, $json_prefix) {
    ...
    foreach($ffs as $ff){
	    if(pathinfo($ff, PATHINFO_EXTENSION) == 'json') {
			...
			$titles = find_custom_recursive($excludes, $json, 'title', 'title');
			$pageDescription = find_custom_recursive($excludes, $json, 'pageDescription', 'pageDescription');
			$titleCaptions = find_custom_recursive($excludes, $json, 'titleCaption', 'titleCaption');
			$fieldCaptions = find_custom_recursive($excludes, $json, 'fieldCaption', 'fieldCaption');
